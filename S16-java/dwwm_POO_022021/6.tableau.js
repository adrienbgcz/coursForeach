...
function afficheMenu() {
    ...
    console.log('16 - exercice 16 - algo aléatoire essayez de deviner')
    console.log('16b - exercice 16 - algo amélioré essayez de deviner')
    console.log('17 - exercice 17 - somme de 2 tableaux')
    console.log('18 - exercice 18 - Schtroumpf')
    console.log('19a - exercice 19 - mario course')
    console.log('19b - exercice 19 - mario course')
    console.log('19c - exercice 19 - mario course')
...
function exercice15() {
    // definir un nombre aléatoire entre 1 et 50
    const numberToFind = getRandomInt(1, 50);

    let nbTentative = 0;
    let numberToTest = 0;
    const tentatives = [];
    // permettre à l'utilisateur d'essayer de le deviner
    while (numberToTest != numberToFind) {
        numberToTest = readlineSync.questionInt('Veuillez saisir votre proposition (1-50)');
        if (numberToTest < 1 || numberToTest > 50) {
            console.log('Le nombre doit être compris entre 1 et 50');
        } else {
            nbTentative += 1;
            tentatives.push(numberToTest);
            if(numberToTest > numberToFind){
                console.log('Votre proposition est trop grande')
            } else if(numberToTest < numberToFind){
                console.log('Votre proposition est trop petite')
            }
        }
    }

    // Resultat : 
    // Trouvé !! vous avez fait 3 tentatives : nb1, nb2, nb3
    console.log(`Vous avez trouvé !! Vous avez fait ${nbTentative}: ${tentatives}`)
}
...
function exercice16() {
    // definir un nombre aléatoire entre 1 et 50
    let min = 1, max = 50;
    const numberToFind = getRandomInt(min, max);
    ...
    // permettre à l'utilisateur d'essayer de le deviner
    while (numberToTest != numberToFind) {
        // on choisi un nombre aléatoire entre les 2 bornes
        numberToTest = getRandomInt(min, max);
        nbTentative += 1;
        tentatives.push(numberToTest);
        // on met a jour les bornes suivant si c'est plus petit ou plus grand
        if(numberToTest > numberToFind){
            max = numberToTest - 1;
        } else if(numberToTest < numberToFind){
            min = numberToTest + 1;
...
function getMiddle(min, max){
    const intervalle = max - min;
    if(intervalle % 2 == 0){
        return min + (intervalle/2)
    }
    return min + ((intervalle + 1)/2)
}

// stratégique
function exercice16b() {
    // definir un nombre aléatoire entre 1 et 50
    let min = 1, max = 50;
    const numberToFind = getRandomInt(min, max);
    ...
    // permettre à l'utilisateur d'essayer de le deviner
    while (numberToTest != numberToFind) {
        // on prend le molieu des 2 bornes
        numberToTest = getMiddle(min, max);
        nbTentative += 1;
        tentatives.push(numberToTest);
        if(numberToTest > numberToFind){
            max = numberToTest - 1;
    ...
    // Resultat : 
    // Trouvé !! vous avez fait 3 tentatives : nb1, nb2, nb3
    console.log(`Vous avez trouvé !! Vous avez fait ${nbTentative} tentatives: ${tentatives}`)
...
function exercice17() {
    const taille = readlineSync.questionInt('Quelle sera la taille de vos tableaux ?')

    const tableau1 = [];
    for(let i = 0; i < taille; i++){
        tableau1[i] = readlineSync.questionInt(`Vauillez saisir la valeur ${i+1} de votre premier tableau: `)
    }

    const tableau2 = [];
    for(let i = 0; i < taille; i++){
        tableau2[i] = readlineSync.questionInt(`Vauillez saisir la valeur ${i+1} de votre deuxième tableau: `)
    }
...
function exercice18() {
    ...
    let taille = readlineSync.questionInt('Quelle sera la taille de votre premier tableau ?')
    const tableau1 = [];
    for(let i = 0; i < taille; i++){
        tableau1[i] = readlineSync.questionInt(`Vauillez saisir la valeur ${i+1} de votre premier tableau: `)
    }

    taille = readlineSync.questionInt('Quelle sera la taille de votre deuxième tableau ?')
    const tableau2 = [];
    for(let i = 0; i < taille; i++){
        tableau2[i] = readlineSync.questionInt(`Vauillez saisir la valeur ${i+1} de votre deuxième tableau: `)
    }
    ...
    for(let i = 0; i < tableau2.length; i++){
        for(let j = 0; j < tableau1.length; j++){
            schtroumpf += tableau2[i] * tableau1[j];
        }
    }
...
function exercice19a() {
    ...
    // for(let i = 1; i < course.length; i = i + 2) {
    //     resultat[i] = points[course[i]]
    // }

    for(let i = 0; i < course.length; i++) {
        if(i%2 != 0){
            resultat[i] = points[course[i]-1];
        }
    }
    console.log(resultat)
}

function exercice19b() {

    const resultat = ['Yoshi', 0, 'Toad', 0, 'Peach', 0, 'Bowser', 0, 'Mario', 0, 'Luigi', 0];
    const points = [6, 4, 3, 2, 1, 0];
    const course = [ 'Yoshi', 6, 'Toad', 3, 'Peach', 2, 'Bowser', 5, 'Mario', 4, 'Luigi', 1];
...
function ajouteResultat(resultat, course, points){
    for(let i = 1; i < course.length; i = i + 2) {
        resultat[i] += points[course[i]-1]
    }
}

function exercice19c() {

    const resultat = ['Yoshi', 0, 'Toad', 0, 'Peach', 0, 'Bowser', 0, 'Mario', 0, 'Luigi', 0];
    const points = [6, 4, 3, 2, 1, 0];

    let course = [ 'Yoshi', 1, 'Toad', 2, 'Peach', 3, 'Bowser', 4, 'Mario', 5, 'Luigi', 6];
    ajouteResultat(resultat, course, points)
    course = [ 'Yoshi', 6, 'Toad', 3, 'Peach', 2, 'Bowser', 5, 'Mario', 4, 'Luigi', 1];
    ajouteResultat(resultat, course, points)
